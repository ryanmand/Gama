...
import { sampleWithRequiredData } from '../aluno.test-samples';
...
describe('Aluno Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.alunos()[0]).toEqual(expect.objectContaining({ id: 15328 }));
  });

  describe('trackId', () => {
    it('Should forward to alunoService', () => {
      const entity = { id: 15328 };
      jest.spyOn(service, 'getAlunoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAlunoIdentifier).toHaveBeenCalledWith(entity);
